...
const MoviesDisplay = ({title}:Props) => {
  ...
  <>
   <div className=' py-5' >
      ...
      <Link to={`/${title}`} className='flex justify-center gap-2 hover:text-accent items-center'>See all <AiOutlineRight/></Link>
      </div>
      <Swiper
        modules={[Navigation]}
        navigation={true}
      ...
      >
        {Array.from({ length: 30 }, (_, index) => index + 1).map((collection, i) => {
          return (
            <SwiperSlide
            ...
            >
              <motion.div
                ...
                transition={{delay: i * 0.07  , bounce:0.4 , stiffness:3000, duration:0.2}}
                // whileHover={{scale:1.2}}
                viewport={{once:true}}
                className=' cursor-pointer  rounded-md overflow-hidden'
              >

               <PopularMoviesCard id={i} image='https://www.themoviedb.org/t/p/original/w50Ofls7O0OoSaEcmzF1sNIZJYF.jpg'/>
              </motion.div>
            </SwiperSlide>
          )
        })}
