...
const Waveform = memo(
  ({ player, setWaveform, setRender }) => {
    ...
    useEffect(() => {
      ...
      const movieasstring = localStorage.getItem("currentMovie");
      const movie = {
        id: 1,
        name: "Bhramastra",
        moviesrc: "http://localhost:5173/movies/bhramastra.mp4",
        subtitle: "http://localhost:5173/movies/bhramastra_sub.json",
        audio: "bhramastra_aud.mp3",
      };

      waveform.load(`/movies/${movie.audio}` || "/movies/raniganj_aud.mp3");
...
const Progress = (props) => {
  ...
  const onProgressClick = useCallback(
    (event) => {
      if (event.button !== 0) return;
      const currentTime =
        (event.pageX / document.body.clientWidth) * props.player.duration;
      props.player.currentTime = currentTime;
      props.waveform.seek(currentTime);
    },
    [props]
  ...
  return (
    <div className="progress" onClick={onProgressClick}>
      <div
        className="bar"
        style={{
          width: `${(props.currentTime / props.player.duration) * 100}%`,
        }}
      >
...
export default function Footer(props) {
  ...
  const onWheel = useCallback(
    (event) => {
      ...
      const currentTime = clamp(
        props.player.currentTime + deltaY,
        0,
        props.player.duration
      );
      props.player.currentTime = currentTime;
  ...
  return (
    <FooterStyle className="footer" ref={$footer}>
      {props.player ? (
        <React.Fragment>
          <Progress {...props} />
          <Duration {...props} />
